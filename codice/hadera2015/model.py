...
def get_hedera_full_model() -> gp.Model:
    ...
    rmin = 3 # hours 
    dmin = 3 # hours
    h = data.power_consumption
    c_weight = 1
    aload = data.load_curve 
    c_cost = data.costi_energia
    cost_gen = 61
    ...
    # DECISION VARIABLES
    X = mdl.addVars(M,P, vtype=GRB.BINARY, name='product_to_machine')
    tf = mdl.addVars(M,P, vtype=GRB.CONTINUOUS, lb=0, name='time_finish')
    ts = mdl.addVars(M,P, vtype=GRB.CONTINUOUS, lb=0, name='time_start')
    tfstage = mdl.addVars(P,ST, vtype=GRB.CONTINUOUS, lb=0, name='time_finish_stage') 
    tsstage = mdl.addVars(P,ST, vtype=GRB.CONTINUOUS, lb=0, name='time_start_stage') 
    w = mdl.addVars(P, ST, vtype=GRB.CONTINUOUS, lb=0, name='slack_after_stage') 
    V = mdl.addVars(ST, P, P, vtype=GRB.BINARY, name='precedence')
    ...
    ysaux = mdl.addVars(P,M,ST,S, vtype=GRB.CONTINUOUS, lb=0, name='aux var start')   
    yfaux = mdl.addVars(P,M,ST,S, vtype=GRB.CONTINUOUS, lb=0, name='aux var finish')   
    a = mdl.addVars(P,M,ST,S, vtype=GRB.CONTINUOUS, lb=0, name='task start-end within time slot')  # a,b,c,d indicano i 4 casi del contributo di consumo energia in ciascuno slot
    b = mdl.addVars(P,M,ST,S, vtype=GRB.CONTINUOUS, lb=0, name='task start before and finish within time slot')
    c = mdl.addVars(P,M,ST,S, vtype=GRB.CONTINUOUS, lb=0, name='task start within and finish after time slot')
    d = mdl.addVars(P,M,ST,S, vtype=GRB.CONTINUOUS, lb=0, name='task start before and finish after time slot')
    q = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='energy used in time slot')
    f = mdl.addVars(S,NODE,NODE, vtype=GRB.CONTINUOUS, lb=0, name='flow on arch on time s')
    G = mdl.addVars(S,NODE,NODE, vtype=GRB.BINARY, name='plant is active')
    gs = mdl.addVars(S,NODE,NODE, vtype=GRB.CONTINUOUS, lb=0, name='generator startup')
    cgen = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='on site gen cost')
    muh = mdl.addVar(vtype=GRB.CONTINUOUS, name='total cost energy')
    bo = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='upper bound')
    bu = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='lower bound')
    co = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='over consumption')
    cu = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='lower consumption')
    delta = mdl.addVar(vtype=GRB.CONTINUOUS, lb=0, name='deviation penalty')
    b_buffer = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='buffer')
    ...
    # energy 
    mdl.addConstrs((tsstage[p,st] >= tau[S[i-1]]*ys[p,st,S[i]] for p in P for st in ST for i in range(1, len(S))), name='(17)') # TODO: CHECK time (controllare S, tau e come viene indicizzato in questo vincolo)
    mdl.addConstrs((tsstage[p,st] <= tau[s]+(HORIZON-tau[s])*(1-ys[p,st,s]) for p in P for st in ST for s in S), name='(18)')
    mdl.addConstrs((tfstage[p,st] >= tau[S[i-1]]*yf[p,st,S[i]] for p in P for st in ST for i in range(1, len(S))), name='(19)') # TODO: check time
    ...
    mdl.addConstrs((b[p,m,st,S[i]] >= tf[m,p] - tau[S[i-1]] - (HORIZON - tau[S[i-1]])*(1-yfaux[p,m,st,S[i]]+ysaux[p,m,st,S[i]]) for p in P     
                                                                                                                                for st in ST 
                                                                                                                                    for m in M if m in SM[st] 
                                                                                                                                        for i in range(1, len(S))), name='(30)')
    mdl.addConstrs((b[p,m,st,S[i]] <= tf[m,p] - tau[S[i-1]] + tau[S[i-1]]*(1-yfaux[p,m,st,S[i]]) for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(31)')
    mdl.addConstrs((b[p,m,st,S[i]] <= (tau[S[i]]-tau[S[i-1]])*yfaux[p,m,st,S[i]] for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(32)') 
    mdl.addConstrs((b[p,m,st,S[i]] <= (tau[S[i]] - tau[S[i-1]])*(1-ysaux[p,m,st,S[i]]) for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(33)')
    mdl.addConstrs((c[p,m,st,s] >= tau[s] - ts[m,p] - tau[s]*(1-ysaux[p,m,st,s]+yfaux[p,m,st,s]) for p in P for st in ST for m in M if m in SM[st] for s in S), name='(34)')
    mdl.addConstrs((c[p,m,st,s] <= tau[s] - ts[m,p] + (HORIZON - tau[s])*(1-ysaux[p,m,st,s]) for p in P for st in ST for m in M if m in SM[st] for s in S), name='(35)') 
    mdl.addConstrs((c[p,m,st,S[i]] <= (tau[S[i]] - tau[S[i-1]])*ysaux[p,m,st,S[i]] for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(36)')
    mdl.addConstrs((c[p,m,st,S[i]] <= (tau[S[i]]-tau[S[i-1]])*(1-yfaux[p,m,st,S[i]]) for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(37)')
    mdl.addConstrs((d[p,m,st,S[i]] >= (tau[S[i]]-tau[S[i-1]])
                                            *(gp.quicksum(ysaux[p,m,st,s1] for s1 in S if int(s1) < int(S[i]))+gp.quicksum(yfaux[p,m,st,s1] for s1 in S if int(s1) > int(S[i]))-1)
                                                 for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(38)')
    mdl.addConstrs((d[p,m,st,S[i]] <= (tau[S[i]] - tau[S[i-1]])*gp.quicksum(ysaux[p,m,st,s1] for s1 in S if int(s1) < int(S[i])) 
                                                                                    for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(39)')
    mdl.addConstrs((d[p,m,st,S[i]] <= (tau[S[i]]-tau[S[i-1]])*gp.quicksum(yfaux[p,m,st,s1] for s1 in S if int(s1) > int(S[i]))
                                                                                    for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(40)')
    mdl.addConstrs((d[p,m,st,S[i]] <= (tau[S[i]]-tau[S[i-1]])*(1-ysaux[p,m,st,S[i]]) for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(41)')
    mdl.addConstrs((d[p,m,st,S[i]] <= (tau[S[i]]-tau[S[i-1]])*(1-yfaux[p,m,st,S[i]]) for p in P for st in ST for m in M if m in SM[st] for i in range(1, len(S))), name='(42)')
    mdl.addConstrs((q[s] == gp.quicksum(h[p][m]*(a[p,m,st,s]*teta[p][m]+b[p,m,st,s]+c[p,m,st,s]+d[p,m,st,s]) for p in P for st in ST for m in M if m in SM[st])/60 for s in S), name='(43)') 
    ...
    mdl.addConstr((gp.quicksum(X[m,p]*h[p][m]*teta[p][m] for p in P for st in ST for m in M if m in SM[st])/60 == gp.quicksum(q[s] for s in S)), name='(46)') 
    # grafo
    ...
    mdl.addConstrs((G[S[i1],i,j] - G[S[i1-1],i,j] <= gs[S[i1],i,j] for i in GEN for j in BAL if j in ARC[i] for i1 in range(1,len(S))), name='(51.1)')
    ...
    mdl.addConstrs((gs[S[i1],i,j] <= 1-G[S[i1-1],i,j] for i in GEN for j in BAL if j in ARC[i] for i1 in range(1,len(S))), name='(52.2)')
    mdl.addConstrs((cgen[s] == gp.quicksum(f[s,i,j]*cost_gen + cstart*gs[s,i,j] for i in GEN for j in BAL if j in ARC[i]) for s in S), name='(53)')
    ...
    mdl.addConstrs((gp.quicksum(G[S[s1],str(i),j] for s1 in range(s2, s2+rmin)) >= rmin*(G[S[s2],str(i),j]-G[S[s2-1],str(i),j]) for i in GEN for j in BAL if j in ARC[i]
                                                                                                for s2 in range(1, len(S)-rmin+1)), name='(55)') # TODO: check indici 
    mdl.addConstrs((gp.quicksum(G[S[s1],str(i),j] for s1 in range(s2, s2+dmin)) <= dmin*(1+G[S[s2],str(i),j]-G[S[s2-1],str(i),j]) for i in GEN for j in BAL if j in ARC[i] 
                                                                                                for s2 in range(1, len(S)-dmin+1)), name='(56)')
    # mdl.addConstr((muh == gp.quicksum(gp.quicksum(f[s,i,j]*c_cost[s][i][j]+cgen[s] for i in NODE for j in PUR if j in ARC[i]) 
    #                                     - gp.quicksum(f[s,i,j]*c_cost[s][i][j]*0.75 for i in NODE for j in SALE if j in ARC[i]) for s in S)), name='(57)')
    mdl.addConstr((muh == gp.quicksum(gp.quicksum(f[s,i,j]*c_cost[s][i][j] for i in NODE for j in PUR if j in ARC[i]) + cgen[s] 
                                        - gp.quicksum(f[s,i,j]*c_cost[s][i][j]*0.75 for i in NODE for j in SALE if j in ARC[i]) for s in S)), name='(57)')

    mdl.addConstrs((-aload[s]*bu[s] <= b_buffer[s] for s in S), name='(58.1)')
    mdl.addConstrs((b_buffer[s] <= aload[s]*bo[s] for s in S), name='(58.2)')
    mdl.addConstrs((q[s] == aload[s] + co[s] - cu[s] + b_buffer[s] for s in S), name='(59)')
    mdl.addConstr((delta == po*gp.quicksum(co[s] for s in S) + pu*gp.quicksum(cu[s] for s in S)), name='(60)')
...
def get_hedera_full_model_simplified() -> gp.Model:
    mdl = gp.Model('hedera_full')
    ...
    tau = data.tau
    HORIZON = 24*60
    costo_energia_3 = data.costi_energia_3
    h = data.power_consumption
    c_weight = 1
    ...
    d = mdl.addVars(P,M,ST,S, vtype=GRB.CONTINUOUS, lb=0, name='task start before and finish after time slot')
    q = mdl.addVars(S, vtype=GRB.CONTINUOUS, lb=0, name='energy used in time slot')
    muh = mdl.addVar(vtype=GRB.CONTINUOUS, lb=0, name='total cost energy')
    ...
    # mdl.addConstrs((gp.quicksum(yf[p,st,s] for s in S) == gp.quicksum(X[m,p] for m in M if m in SM[st]) for p in P for st in ST), name='(45)')
    mdl.addConstr((gp.quicksum(X[m,p]*h[p][m]*teta[p][m] for p in P for st in ST for m in M if m in SM[st])/60 == gp.quicksum(q[s] for s in S)), name='(46)') 
    # grafo (tutta l'energia necessaria per la macchina è acquistata dal mercato day ahead, non si vende energia)
    mdl.addConstr((muh == gp.quicksum(q[s]*costo_energia_3[int(s)-1] for s in S)), name='(47)')
    
    # OBJECTIVE FUNCTION
    mdl.setObjective(muh + c_weight*gp.quicksum(ts[m, p] for m in M for p in P), GRB.MINIMIZE) 
    return mdl
